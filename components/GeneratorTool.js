...
import styles from './GeneratorTool.module.css';
...
/**
 * The generator tool: format tabs, upload, per-format fields, and the canvas.
 *
 * Extracted from the `/preview` route so it can be composed under the homepage
 * hero as well. It does NOT own the frames counter — a single owner higher up
 * passes `count`/`onBump`, because two `useCounter()` calls desync the moment
 * `bump()` fires and the hero would show a stale number right after the user's
 * own download incremented it.
 *
 * @param as        root element ('main' when it is the page, 'section' when the
 *                  hero above it owns the <main>)
 * @param brandTag  heading level for the wordmark — 'h1' standalone, 'h2' when
 *                  the hero's headline is already the page's h1
 */
export default function GeneratorTool({
  as: Root = 'main',
  brandTag: BrandTag = 'h1',
  id,
  count,
  onBump,
}) {
  ...
  return (
    <Root className={styles.wrap} id={id} tabIndex={id ? -1 : undefined}>
      <header className={styles.topRow}>
        <div className={styles.brandLockup}>
          <BrandTag className={styles.brandTitle}>HACKER HOUSE GOA</BrandTag>
