...
export function drawTravelStickersCollage(
  ...
  // 6. VIP Gold-Foil Wax Seal (VERIFIED BUILDER · HH GOA 2026)
  if (enabled('waxSeal')) {
    drawVipWaxSeal(ctx, fonts, {
      cx: 240,
      cy: 300,
      r: 65,
  ...
  // 7. GOA BOUND 🚀 Passport Customs Stamp
  if (enabled('goaBound')) {
    drawGoaBoundStamp(ctx, fonts, {
      cx: 860,
      cy: 1220,
      w: 220,
      h: 90,
...
/* 6. VIP Gold-Foil Wax Seal (VERIFIED BUILDER 2026) ------------------------- */

function drawVipWaxSeal(ctx, fonts, { cx, cy, r, angle }) {
  ctx.save();
  ctx.translate(cx, cy);
  ctx.rotate(angle);
  ...
  // Center Emblem: Rocket Crown
  ctx.fillStyle = '#423403';
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ...
  ctx.font = `900 11px ${fonts.display}`;
  ctx.fillText('VERIFIED', 0, 10);

  ctx.font = `700 8px ${fonts.mono}`;
  ctx.fillText('BUILDER 2026', 0, 22);

  ctx.restore();
}

/* 7. GOA BOUND 🚀 Airport Customs Stamp -------------------------------------- */

function drawGoaBoundStamp(ctx, fonts, { cx, cy, w, h, angle }) {
  ctx.save();
  ctx.translate(cx, cy);
  ctx.rotate(angle);
  ...
  ctx.font = `700 11px ${fonts.mono}`;
  ctx.fillStyle = PALETTE.cream;
  ctx.fillText('★ PASSED CUSTOMS ★', 0, -hh + 18);

  ctx.font = `900 20px ${fonts.display}`;
  ctx.fillStyle = PALETTE.yellow;
  ctx.fillText('GOA BOUND 🚀', 0, 2);

  ctx.font = `700 11px ${fonts.mono}`;
  ctx.fillStyle = PALETTE.pink;
  ctx.fillText('DEC 2026 • OFFICIAL', 0, hh - 18);
