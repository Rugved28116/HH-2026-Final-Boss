...
import { getPfpFrameBgImage } from './bgImage';
import {
...
export function drawPfpFrame(ctx, { fonts, photo = null, photoTransform = null }) {
  ...
  const bgImg = getPfpFrameBgImage();
  if (bgImg && bgImg.complete && bgImg.naturalWidth > 0) {
    ctx.drawImage(bgImg, 0, 0, PFP_SIZE, PFP_SIZE);
    if (photo) {
      drawPhotoOnly(ctx, { cx: CX, cy: CY, photoR: PHOTO_R - 2, photoTransform }, photo);
    }
  } else {
...
function drawPhotoOnly(ctx, { cx, cy, photoR, photoTransform }, photo) {
  ctx.save();
  ctx.beginPath();
