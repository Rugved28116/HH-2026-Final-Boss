 ...
 *
 * Radius is derived from the count, not fixed: four members across 1060px of
 * usable width is the binding constraint, and a radius that fits four would
 * leave one or two looking lost. Height caps it the rest of the time so the
 * labels always clear the footer.
 */
export function memberGeometry(count) {
  ...
  // to Formats A and B at any size; the floors keep them visible at four-up.
 ...
 *   1–4 slots. A slot with `photo: null` renders its placeholder rather than
